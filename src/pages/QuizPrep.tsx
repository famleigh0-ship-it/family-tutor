...
export default function QuizPrep() {
  ...
  return (
    <div className="min-h-screen animate-[fadeIn_150ms_ease-in] bg-slate-50 dark:bg-slate-950">
      <TopBar title="FALP" />
      <main className="mx-auto max-w-sm px-4 py-8">
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-50">{pack.name} — Quiz Prep</h1>

        <div className="mt-6">
          {step === 'loading' && <p className="text-slate-500 dark:text-slate-400">Loading...</p>}
          ...
          {step === 'saving' && <p className="text-slate-500 dark:text-slate-400">Saving...</p>}
          ...
          {step === 'confirm' && (
            <div className="space-y-4 text-center [animation:fadeIn_300ms_ease-in]">
              <p className="text-lg font-medium text-slate-900 dark:text-slate-50">Quiz prep is set!</p>

              <div className="space-y-2 rounded-lg bg-slate-100 p-4 text-left text-sm text-slate-700 dark:bg-slate-900 dark:text-slate-300">
                ...
                <p className="flex flex-wrap gap-1.5">
                  ...
                  {selectedTopicNames.map((name) => (
                    <span key={name} className="rounded-full bg-white px-2 py-0.5 text-xs dark:bg-slate-800 dark:text-slate-100">
                      {name}
                    </span>
                  ))}
                </p>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Your next session will focus on these topics.</p>
